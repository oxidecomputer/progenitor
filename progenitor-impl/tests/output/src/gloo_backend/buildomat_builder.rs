...
pub mod builder {
    ...
    impl<'a> TaskEventsGet<'a> {
        ...
        pub async fn send(
            ...
            let request = ::gloo_net::http::Request::get(&url)
                .header("accept", "application/json")
                .query(
                    ::serde_urlencoded::to_string([("minseq", &minseq)])
    ...
    pub struct WhoamiPutName<'a> {
        client: &'a super::Client,
        body: Result<::wasm_bindgen::JsValue, String>,
    }
    ...
    impl<'a> WorkerTaskUploadChunk<'a> {
        ...
        where
            B: std::convert::TryInto<::wasm_bindgen::JsValue>,
        {
            self.body = value
                .try_into()
                .map_err(|_| "conversion to `wasm_bindgen::JsValue` for body failed".to_string());
            self
        }
